...
#[test]
fn expr_rel() {
    let actual = semic::parse_expr(r#"s == "foo" || i < 9"#);
...
#[test]
fn expr_float() {
    let actual = semic::parse_expr(r#"0.0"#);
    let expected = r#"0.00"#;
    assert!(actual.is_ok());
    assert_eq!(expected, format!("{:?}", actual.unwrap()));
}
